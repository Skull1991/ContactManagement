...
def delete():
    if (employeeID.get()==""):
        ...
        conn = sqlite3.connect('ContactInfo.db')
        c = conn.cursor()
        c.execute('DELETE from addresses WHERE oid= ' + employeeID.get())
...
def search():
    ...
    c.execute("SELECT rowid, * FROM addresses WHERE First_Name = ?", (record_id,))
    records = c.fetchall()
...
addEmpBTN=Button(root,text="ADD CONTACT",font=('Consolas',13),cursor='hand2',
                  bg="#00bff3",border=0,activebackground="#00bff3",padx=50,command=adding)
